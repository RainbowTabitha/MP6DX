...
wine "compiler/codewrite/powerpc-gekko-as.exe" -a32 -mbig -mregnames -mgekko "src/asm/MP6DX 14 - Koopa Orb Rewrite.asm" && python "compiler/gecko.py" a.out 801BE72C tmp/14.txt1

# Copy the text file
cp "src/asm/MP6DX 15 - Star Blocking.txt" tmp/15.txt1
cp "src/asm/MP6DX 16 - Orb Tables.txt" tmp/16.txt1
